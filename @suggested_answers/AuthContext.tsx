// @suggested_answers/AuthContext.tsx
...
import { api, setAuthToken } from "./api"; // Import the 'api' instance
...
export const AuthProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const logout = async () => {
    ...
    // Before logging out, make a best-effort attempt to set the driver's
    // status to 'offline' on the backend.
    try {
      await api.post('/driver/status', { status: 'offline' });
    } catch (error) {
      // Log a warning but don't block the logout process if the call fails.
      // The user should still be able to log out of the device.
      console.warn('Failed to set driver status to offline on logout:', error);
    }
    ...
    // Remove credentials from persistent storage
